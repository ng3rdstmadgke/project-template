...
- `/cognitive-rhythm-writing/` スキルの方針に従い、人間とAIどちらも理解しやすいように記述すること
...
- `spec.md` / `plan` を書かずに実装を始めようとしている
...
- 設計(`spec`)を保存する場所: `agent-tasks/<ブランチ名>/<TOPIC>/spec.md`
...
- ブレインストーミングでは要件定義->基本設計->詳細設計の順でそれぞれ必要事項を話し合う
  ...
  - 基本設計 (アーキテクチャ)
    ...
    - ロギング
    - バッチ・ジョブ設計
    - UIUX設計 : `/superpowers:brainstorming` の 「Visual Companion」 を利用すること
    - DBの論理設計
...
spec.mdの内容をユーザーが確認する。修正事項などがなければ「2. 実装計画」に進む
...
flowchart TD
    ...
    F --> G[fixesに記録]
    G -->|次の指摘| A
    G -->|4.6 ユーザーが完了と判断| H[閉じる<br>総点検、ADR、教訓]
```
...
| ADR、教訓 | 4.7でまとめて整理する(修正ごとには触らない) |
